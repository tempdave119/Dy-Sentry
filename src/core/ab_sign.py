"""a_bogus 签名（HTTP API 反爬）。纯 Python，仅依赖标准库 math/time。

平台更新算法导致拉流/状态请求失败时，修复点集中在本文件的常量与字节布局
（window_env_str、aid/pageId、b[] 布局、encoding_tables）。算法移植自只读存档
DLR:src/ab_sign.py，逐字保留以确保正确性。
"""
import math
import time
